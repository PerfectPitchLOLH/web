...
async function testUpgrade() {
  ...
  console.log('=== TEST 4: UPGRADE BASIC → PRO ===\n')
  ...
  const { stripe: stripeClient } = await import('../src/server/lib/stripe')
  ...
  if (afterSub!.plan.transcriptionMinutes === 50) {
    console.log('✅ Plan upgraded to Pro (50 min)')
  } else {
    console.log(
      `❌ Plan not upgraded: ${afterSub!.plan.transcriptionMinutes} min`,
    )
  }
